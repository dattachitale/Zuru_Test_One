...
def decision_function(list_format=False, all_files=False, reverse=False, time_modify=False, filter=False, human=False):
    """

    This is a decision function where it decides which function to call depending on the filter used by user.
...
def main_function():
    ...
    This function is for adding new command arguments to argparser class. This checks which all flags are set to true and calls decision_function() or top_directory() accordingly
    ...
    parser.add_argument('-A', '--all', action='store_true',
                        help='list all the files and directories (including files starting with .)')
    parser.add_argument('-r', '--reverse', action='store_true',
                        help='Reverse list of directories with additional information')
    parser.add_argument('-t', '--time_modify', action='store_true',
                        help='Show directories on time modified filter but in reversed order')
    parser.add_argument('-filter', '--filter', metavar='FILTER_OPTION', type=str, choices=['file', 'dir'],
                        help='filter the output on the basis of (file or dir)')
    parser.add_argument('-p', '--path',
                        help='Handle Paths - it gives detail info about subfiles if filter-option is dir and file itself of the filter-option is file')
    parser.add_argument('-H', '--human', nargs='?', const=True,
                        help='Human readable format for Size parameter of the file or dir provided by user')
    ...
    if args.list or args.all or args.reverse or args.time_modify or args.filter or args.path or args.human:
        decision_function(list_format=args.list, all_files=args.all, reverse=args.reverse, time_modify=args.time_modify,
                      filter=args.filter, human=args.human)
    else:
